...
use crate::{
    trading_venue::{QuoteRequest, TradingVenue, error::TradingVenueError, protocol::PoolProtocol},
    your_venue::Deriverse,
};
...
pub enum Venue {
    RaydiumAmm,
    Deriverse { instr_id: u32, input_crncy: u8 },
}
...
pub fn protocol_to_venue(
    ...
    match venue.protocol() {
        ...
        // For instruction construction deriverse need instr_id which can be derived only from pool account data.
        // Current implementation use PoolProtocol as a placeholder for instr_id, expected workflow
        // 1. let deriverse = Deriverse::from_account(..)
        // 2. deriverse.update_state(..)
        // 3. protocol_to_venue(&deriverse, ..)
        //
        // Note: It seems like PoolProtocol enum is currently used as identification enum and should not store
        // any additional data
        // instr_id can be removed from with 2 variants:
        // 1. Moving protocol_to_venue in TradingVenue trait
        // 2. Calling venue.generate_swap_instruction again, constructing SwapData from instruction.data, taking instr_id from it
        PoolProtocol::Deriverse { instr_id } => {
            let b_token = venue.get_token_info().get(Deriverse::TOKEN_B_INDEX).ok_or(
                TradingVenueError::TokenInfoIndexError(Deriverse::TOKEN_B_INDEX),
            )?;

            let input_crncy = b_token.pubkey == request.input_mint;
