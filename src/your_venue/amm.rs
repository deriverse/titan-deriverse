...
impl DeriverseAmm {
    ...
    pub fn get_amm_qty(&self, price: i64, side: OrderSide) -> Result<CappedI64, TradingVenueError> {
    ...
    pub fn get_amm_px<T: Into<i64>>(
        ...
        Ok(match side {
            OrderSide::Bid => {
                let new_tokens = (self.a_tokens.value.checked_add(q).ok_or(
                    TradingVenueError::CheckedMathError(ErrorInfo::StaticStr("Checked add error")),
                )?) as i128;
            ...
            OrderSide::Ask => {
                if q >= self.a_tokens.value {
                    ...
                    let new_tokens = (self.a_tokens.value.checked_sub(q).ok_or(
                        TradingVenueError::CheckedMathError(ErrorInfo::StaticStr(
                            "Checked add error",
                        )),
                    )?) as i128;
                    (((self.k as f64) * self.df) / (new_tokens * new_tokens) as f64) as i64
    ...
    pub fn get_reversed_amm_px<T: Into<i64>>(&self, sum: T) -> Result<i64, TradingVenueError> {
        if self.b_tokens == 0 {
            Ok(i64::MAX >> 1)
        } else {
            let new_crncy =
    ...
    pub fn get_reversed_amm_qty<T: Into<i64>>(
        &self,
        traded_sum: T,
    ) -> Result<CappedI64, TradingVenueError> {
        ...
        if self.b_tokens == 0 {
            ...
            let new_crncy = (self.b_tokens.checked_add(traded_sum).ok_or(
                TradingVenueError::CheckedMathError(ErrorInfo::StaticStr("Checked add error")),
            )?)
            .value as i128;
    ...
    pub fn get_reversed_amm_sum(&self, price: i64) -> Result<CappedI64, TradingVenueError> {
        if self.b_tokens == 0 {
            Ok(CappedI64::new(0))
        } else {
